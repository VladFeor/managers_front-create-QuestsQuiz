<template>
  ...
  <Modal v-if="show" @close="show = false">
    ...
    <div class="counts">Manager has joined {{ countOfProjects }} projects</div>
    <hr class="line" />
    <div class="projects">
      <div class="search">
...
export default {
  ...
  setup() {
    ...
    const projects = computed(() => managersStore.getProjects);
    const manager = computed(() => managersStore.getManager);
    const search = ref('');
    const remove = (index) => {
      AxiosService.delete(
        useRuntimeConfig().public.apiBaseV2 +
          `managers/${manager.value.id}/invites/${index.project.id}`,
      )
        .then(async () => {
    ...
    const kick = (index) => {
      AxiosService.delete(
        useRuntimeConfig().public.apiBaseV2 +
          `managers/${manager.value.id}/projects/${index.project.id}`,
      )
        .then(async () => {
          await managersStore.getManagerProjectsByIdAction(manager.value.id);
          await managersStore.getManagerByIdAction(manager.value.id);
    ...
    const searchInProject = () => {
      ...
      timeOut.value = setTimeout(() => {
        managersStore.getManagerProjectsByIdAction(manager.value.id, search.value);
      }, 300);
    };
    return {
      projects,
