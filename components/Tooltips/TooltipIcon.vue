<template>
  <div class="tooltip-icon" @mouseover="isHovered = true" @mouseleave="isHovered = false">
    <div class="tooltip-checkbox" :class="{ isLeft, isVertical }" v-if="isHovered && tooltipText">
      {{ props.tooltipText }}
    </div>

    <!-- <Icon class="hovered" name="Info-1px-color" :size="24" /> -->
    <Icon class="hovered" name="Info-1px-color" :size="24" />
    <img v-if="isWhite && !isHovered" src="@/assets/images/wallet/wallet-info.svg" alt="" />
    <Icon v-else class="def" name="Info-1px" :size="24" />
  </div>
</template>
