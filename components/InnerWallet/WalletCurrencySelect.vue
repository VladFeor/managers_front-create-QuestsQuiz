<template>
  ...
  >
    <div class="selected" :class="{ open }" @click="toggle">
      <span>{{
        reduceStringLength(
          isToken && selected && selected.symbol
            ? selected.symbol
      ...
      <span
        v-if="
          ...
          (walletType === ConnectionType.WALLET_CONNECT && !props.chainId)
        "
        class="arrow-task"
        :class="{ flipped: open }"
      ></span>
      <TooltipIcon v-else tooltipText="Go to your wallet app to switch the blockchain." isVertical />
      <!-- <TooltipIcon
        v-if="walletType === ConnectionType.WALLET_CONNECT && !props.chainId"
        tooltipText="Go to your wallet app to switch the blockchain."
      /> -->
    </div>
    <div class="items" :class="{ selectHide: !open, isScroll, vertical }">
      ...
      <div
        ...
        "
      >
        <div class="token-data_wrapper">
          <div class="token-data">
            <div class="token-data-content">
              {{ reduceStringLength(option.name, props.reduceLengthName) }}
            ...
            <div
              ...
              class="chain-error"
            >
              Switch chain in your wallet
            </div>
          </div>

          <Icon
            v-if="isToken && selected.address === option.address"
            name="Tik"
            :size="20"
            color="#38405B"
          />
          <Icon
            v-if="!isToken && selected.id === option.id"
            name="Tik"
            :size="20"
            color="#38405B"
          />
        ...
        <!-- <div
          v-if="
            chainId && option.chain_id !== chainId && walletType === ConnectionType.WALLET_CONNECT
          "
          class="chain-error"
        >
          Switch chain in your wallet
...
const toggle = () => {
  if (props.chainId && walletType.value === ConnectionType.WALLET_CONNECT) return;
  if (props.isDisabled) return;
  open.value = !open.value;
};

const selectOption = (option, isStop = false) => {
  if (isStop) return;
  if (props.filter) {
...
//     color: $default-border !important;
//     border-radius: 8px;
...
.custom-select-token .selected.open {
  background: $white;
...
.custom-select-token .selected:after {
  position: absolute;
  content: '';
  top: 12px;
  right: 1em;
